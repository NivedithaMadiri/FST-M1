...
REM   Aggregate functions-GroupBy
...
select max(purchase_amount) as maxamount,salesman_id,order_date from orders where order_date ='17-08-2012' group by salesman_id,order_date;

select max(purchase_amount) as maxamount,customer_id,order_date from orders  group by customer_id,order_date having max(purchase_amount) in (2030, 3450, 5760, 6000) ;

select * from orders;

select max(purchase_amount) as maxamount,customer_id from orders group by customer_id;

select max(purchase_amount) as maxamount,salesman_id,order_date from orders where order_date =To_DATE('2012/08/17', 'YYYY/MM/DD') group by salesman_id,order_date;
